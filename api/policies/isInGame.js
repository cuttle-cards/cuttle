  ...
  if (req.session.game) {
  	if (typeof(req.session.game) === "number") {
    	return next();
  		...
  		return res.forbidden(new Error("Error with user's session: 'game' must be an integer"));
  	}
  } else {
  	return res.forbidden(new Error("Session Error: You must join a game to perform that move"));
  }
};
