...
module.exports = {
	...
	draw: function (req, res) {
		...
		.then(function changeAndSave (values) {
			...
			user.frozenId = null;
			game.log.push(user.email + " Drew a card");
			game.turn++;
			var saveGame = gameService.saveGame({game: game})		;
			var saveUser = userService.saveUser({user: user});
			return Promise.all([saveGame, saveUser]);
		...
		.then(function changeAndSave (values) {
			...
			if ( (game.turn % 2) === player.pNum) {
				...
				if (!game.topCard) {
					...
					game.turn++;
					game.passes++;
					game.log.push(player.email + " passes.");
				} else {
				...
				if (card.hand === player.id) {
						if (card.rank <= 10) {
							if (player.frozenId != card.id) {
								...
								player.frozenId = null;
								game.log.push(player.email + " played the " + card.name + " for points");
								game.passes = 0;
								game.turn++;
								var saveGame = gameService.saveGame({game: game});
								var savePlayer = userService.saveUser({user: player});
				...
				if (card.hand === player.id) {
					if ((card.rank >= 12 && card.rank <= 13) || card.rank === 8) {
						if (player.frozenId != card.id) {
							...
							player.hand.remove(card.id);
							player.frozenId = null;
							game.log.push(player.email + " played the " + card.name + " as a rune");
							game.passes = 0;
							game.turn++;
							var saveGame = gameService.saveGame({game: game});
							var savePlayer = userService.saveUser({user: player});
					...
					if (target.points === opponent.id) {
						if (card.rank > target.rank || (card.rank === target.rank && card.suit > target.suit)) {
							if (player.frozenId != card.id) {
								...
								player.hand.remove(card.id);
								player.frozenId = null;
								game.log.push(player.email + " scuttled " + opponent.email + "'s " + target.name + " with the " + card.name);
								game.passes = 0;
								game.turn++;
								// Save changes
								var saveGame = gameService.saveGame({game: game});
						...
						if (target.points === opponent.id) {
							...
							if (queenCount === 0) {
								if (player.frozenId != card.id) {
									...
									game.log.push(player.email + " stole " + opponent.email + "'s " + target.name + " with the " + card.name);
									game.passes = 0;
									game.turn++;
									var saveGame = gameService.saveGame({game: game});
									var savePlayer = userService.saveUser({user: player});
						...
						switch (card.rank) {
							...
							case 7:
								...
								if (player.frozenId != card.id) {
									game.oneOff = card;
									player.hand.remove(card.id);
									game.log.push(player.email + " played the " + card.name + " as a " + card.ruleText);
									var saveGame = gameService.saveGame({game: game});
									var savePlayer = userService.saveUser({user: player});
									return Promise.all([saveGame, savePlayer]);
								} else {
					...
					if (card.hand === player.id) {
						if (card.rank === 2 || card.rank === 9) {
							...
							if (player.frozenId != card.id) {
								...
								game.log.push(player.email + " played the " + card.name + " as a " + card.ruleText + ", targeting the " + target.name);							
								var saveGame = gameService.saveGame({game: game});
								var savePlayer = userService.saveUser({user: player});
								return Promise.all([saveGame, savePlayer]);
							} else {
						...
						if (card.rank === 2) {
							if (!opHasQueen) {
								...
								if (game.twos.length > 0) {
									game.log.push(player.email + " played the " + card.name + " to COUNTER " + opponent.email + "'s " + game.twos[game.twos.length - 1].name + ".");
								} else {
									game.log.push(player.email + " played the " + card.name + " to COUNTER " + opponent.email + "'s " +  game.oneOff.name + ".");
								}
		...
		.then(function changeAndSave (values) {
			...
			if (game.twos.length % 2 === 1) {
				...
				switch (game.oneOff.rank) {
					...
					case 3:
						game.log.push("The " + game.oneOff.name + " one-off resolves; " + player.email + " will draw one card of her choice from the SCRAP pile");
						break;
					case 4:
						game.log.push("The " + game.oneOff.name + " one-off resolves; " + opponent.email + " must discard two cards");
						break;
					case 5:
						//Draw top card
						...
						if (handLen < 7) {						
							...
							if (game.secondCard) {
								game.log.push("The " + game.oneOff.name + " one-off resolves; " + player.email + " draws two cards.");
								...
								game.log.push("The " + game.oneOff.name + " one-off resolves; " + player.email + " draws the last card.");
							}
							...
							if (game.secondCard) {
								...
								if (game.deck.length > 0) {
									game.log.push("The " + game.oneOff.name + " one-off resolves; " + player.email + " draws one card to reach the hand limit.");							
									min = 0;
									max = game.deck.length - 1;
									random = Math.floor((Math.random() * ((max + 1) - min)) + min);
									game.secondCard = game.deck[random].id;
									...
									game.log.push("The " + game.oneOff.name + " one-off resolves; " + player.email + " draws one card (last in deck) to reach the hand limit.");
								}						
							}
						}
						game.passes = 0;
					...
					case 9:
						...
						game.log.push("The " + game.oneOff.name + " resolves on the" + game.oneOffTarget.name + ". The " + game.oneOffTarget.name + " is returned to " + opponent.email + "'s hand, and she may not play it next turn" );
	...
	resolveFour: function (req, res) {
		...
		.then(function changeAndSave (values) {
			...
			if (card2 != null) {
				...
				game.log.push(player.email + " discarded the " + card1.name + " and the " + card2.name + ".");
			} else {
				game.log.push(player.email + " discarded the " + card1.name + ".");
			}
			game.passes = 0;
			game.turn++;
			var saveGame = gameService.saveGame({game: game});
	...
	resolveThree: function (req, res) {
		...
		.then(function changeAndSave (values) {
			...
			game.scrap.add(game.oneOff.id);
			game.oneOff = null;
			game.log.push(player.email + " took the " + card.name + " from the scrap pile to her hand");
			game.passes = 0;
			game.turn++;
			//Save changes
			var saveGame = gameService.saveGame({game: game});
			...
			if (game.turn % 2 === player.pNum) {
				if (game.topCard.id === card.id || game.secondCard.id === card.id) {
					if (card.rank < 11) {
						...
						game.log.push(player.email + " played the " + card.name + " off the top of the deck as points");
						game.passes = 0;
						game.turn++;	
						var saveGame = gameService.saveGame({game: game})					;
						var savePlayer = userService.saveUser({user: player});
			...
			if (game.turn % 2 === player.pNum) {
				if (game.topCard.id === card.id || game.secondCard.id === card.id) {
					if (card.rank === 12 || card.rank === 13 || card.rank === 8) {
						...
						player.frozenId = null;
						game = gameService.sevenCleanUp({game: game, index: req.body.index});
						game.log.push(player.email + " played the " + card.name + " off the top of the deck, as a rune");
						game.passes = 0;
						game.turn++;
						var saveGame = gameService.saveGame({game: game});
						var savePlayer = userService.saveUser({user: player});
					...
					if (card.rank < 11) {
						if (target.points === opponent.id) {
							if (card.rank > target.rank || (card.rank === target.rank && card.suit > target.suit)) {
								...
								game.scrap.add(card.id);
								game = gameService.sevenCleanUp({game: game, index: req.body.index});
								game.log.push(player.email + " scuttled " + opponent.email + "'s " + target.name + " with the " + card.name + " from the top of the deck");
								game.passes = 0;
								game.turn++;
								var saveGame = gameService.saveGame({game: game});
								var savePlayer = userService.saveUser({user: player});
				...
				if (card.id === game.topCard.id || card.id === game.secondCard.id) {
					if (target.points === opponent.id) {
						if (card.rank === 11) {
							...
							player.frozenId = null;
							game = gameService.sevenCleanUp({game: game, index: req.body.index});
							game.log.push(player.email + " stole " + opponent.email + "'s " + target.name + " with the " + card.name + " from the top of the deck");
							game.passes = 0;
							game.turn++;
							var saveGame = gameService.saveGame({game: game});
							var savePlayer = userService.saveUser({user: player});
				...
				if (game.topCard.id === card.id || game.secondCard.id === card.id) {
					switch (card.rank) {
						...
						case 7:
							...
							game.oneOff = card;
							game = gameService.sevenCleanUp({game: game, index: req.body.index});
							game.log.push(player.email + " played the " + card.name + " from the top of the deck as a " + card.ruleText);
							var saveGame = gameService.saveGame({game: game});
							var savePlayer = userService.saveUser({user: player});
							return Promise.all([saveGame, savePlayer]);
							break;
