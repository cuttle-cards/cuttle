              ...
              case 7:
                ...
                switch (card.rank) {
                  case 3:
                    if (game.scrap.length < 1)
                      return Promise.reject({
                        message:
                          'You can only play a 3 as a one-off, if there are cards in the scrap pile',
                      });
                    break;
                  case 4:
                  ...
                  case 7:
                    if (!game.topCard)
                      return Promise.reject({
                        message:
                          "You can't play that card as a one-off, unless there are cards in the deck",
                      });
                    break;
                  default:
                    break;
                ...
                if (player.frozenId !== card.id) {
                  ...
                  const gameUpdates = {
                    ...
                    log: [
                      ...game.log,
                      `${player.username} played the ${card.name} as a one-off to ${card.ruleText}.`,
                    ],
                    lastEvent: {
                      change: 'oneOff',
