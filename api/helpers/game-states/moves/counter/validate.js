  ...
  fn: ({ currentState, requestedMove, playedBy }, exits) => {
    try {
      ...
      if (currentState.phase !== GamePhase.COUNTERING) {
        throw new Error('game.snackbar.global.notCounteringPhase');
      }

      if (!currentState.oneOff) {
        throw new Error('game.snackbar.global.noOneOffPending');
      }
      ...
      if (!yourTurnToCounter) {
        throw new Error('game.snackbar.global.waitForOpponent');
      }
