  ...
  fn: ({ currentState, playedBy }, exits) => {
    try {
      ...
      if (currentState.phase !== GamePhase.COUNTERING) {
        throw new Error('game.snackbar.counter.notInCounteringPhase');
      }

      if (!currentState.oneOff) {
        throw new Error('game.snackbar.resolve.pendingOneOff');
      }
      ...
      if (!yourTurnToResolve) {
        throw new Error('game.snackbar.resolve.waitingForCounter');
      }
