  ...
  fn: ({ gameStateRow }, exits) => {
    try {
      ...
      attributesToConvert.forEach((attribute) => {
        ...
        if (value) {
          ...
          convertedData[attribute] = Array.isArray(value)
            ? value.map((cardStr) => convertStrToCard(cardStr))
      ...
      const data = {
        ...
        targetCardTwo: convertedData.targetCardTwo ?? null,
        ...
        oneOffTargetTwo: convertedData.oneOffTargetTwo ?? null,
        oneOffTargetTwoType: gameStateRow.oneOffTargetTwoType ?? null,
