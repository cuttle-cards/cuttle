  ...
  fn: async ({ game }, exits) => {
    try {
      if (game.status !== GameStatus.STARTED) {
        return exits.error('game.snackbar.global.gameStatusInvalid');
      }

      if (!game.gameStates) {
        return exits.error('game.snackbar.global.missingDotGameStates');
      }

      if (game.gameStates.length) {
        return exits.error('game.snackbar.global.cardsAlreadyDealt');
      }
