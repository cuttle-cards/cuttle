...
description: >-
  ...
  `announcement` i18n block across all five translation files. Follows the pattern of PRs #1183 /
  #1184.
---
...
`announcementData` object, and (3) replace the flat `announcement` i18n block in **all five**
translation files. **Do not invent new component props** — reuse the fields documented below. Only
...
## Step 3 — Replace the `announcement` i18n block in all five files

Files: `src/translations/{en,de,es,fr,ukr}.json`. The `announcement` object is **flat** (string
keys only) and sits just before the `profile` block in each file. Replace its contents so that
...
- Remove any now-unused keys from the previous announcement (e.g. a `twitchLink` that the new
  announcement doesn't use) from all five files, so the block matches the new data exactly.
- After editing, confirm each file still parses as JSON.

## Step 4 — Verify

- JSON valid in all five translation files.
