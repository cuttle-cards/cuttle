  ...
  <BaseDialog id="game-over-dialog" v-model="show">
    ...
    <template #body>
      ...
      <template v-if="currentMatch">
        <p v-if="currentMatch" class="dialog-text" data-cy="match-result-section">
          ...
          {{ this.t('game.dialogs.gameOverDialog.matchAgainst') }} {{ gameStore.opponent.username }}
          <span>: {{ matchIsOver ? this.t('game.dialogs.gameOverDialog.finished') : this.t('game.dialogs.gameOverDialog.inProgress') }}</span>
        </p>
        <p v-if="matchIsOver" class="dialog-text" data-cy="match-winner-message">
          <!-- You won/lost your game against  -->
          {{ this.t('game.dialogs.gameOverDialog.you') }} {{ playerWinsMatch ? this.t('game.dialogs.gameOverDialog.won') : this.t('game.dialogs.gameOverDialog.lost') }} {{ this.t('game.dialogs.gameOverDialog.yourGameAgainst') }} {{ gameStore.opponent.username }}
        </p>
    ...
    <template #actions>
      <v-btn
        ...
        @click="goHome"
      >
      {{ this.t('game.dialogs.gameOverDialog.goHome') }}
      </v-btn>
    </template>
  </BaseDialog>
</template>
