...
describe('Policy Test', function() {
  ...
  it('hasCardId', function() {
    ...
    hasCardId(req, res, next).should.equal(
      false,
      'Passed hasCardId while missing session.cardId property'
    );
  ...
  it('hasCardIdOneAndTwo', function() {
    ...
    req.body.cardId1 = 4;
    req.body.cardId2 = 3;
    hasCardIdOneAndTwo(req, res, next).should.equal(
      true,
      'Valid request failed hasCardIdOneAndTwo'
    );
    // Invalid request (cardId1 is not a number)
    req.body.cardId1 = '4';
    hasCardIdOneAndTwo(req, res, next).should.equal(
      false,
      'Passed hasCardIdOneAndTwo while req.body.cardId1 is not a number'
    );
    // Invalid request (missing cardId1)
    delete req.body.cardId1;
    hasCardIdOneAndTwo(req, res, next).should.equal(
      false,
      'Passed hasCardIdOneAndTwo while missing cardId1 property'
    );
    ...
    hasCardIdOneAndTwo(req, res, next).should.equal(
      false,
      'Passed hasCardIdOneAndTwo while req.body.cardId2 was not a number'
    );
    // Invalid request (missing cardId2)
    delete req.body.cardId2;
    hasCardIdOneAndTwo(req, res, next).should.equal(
      false,
      'Passed hasCardIdOneAndTwo while missing req.body.cardId2'
    );
  });
  ...
  it('hasGameName', function() {
    ...
    req.body.gameName = 4;
    hasGameName(req, res, next).should.equal(
      false,
      'Passed hasGameName while req.body.gameName is not a string'
    );
    // Invalid request (req.body.gameName missing)
    delete req.body.gameName;
    hasGameName(req, res, next).should.equal(
      false,
      'Passed hasGameName while missing req.body.gameName'
    );
  });
  ...
  it('hasOpId', function() {
    ...
    req.body.opId = true;
    hasOpId(req, res, next).should.equal(
      false,
      'Passed hasOpId while req.body.opId was not a number'
    );
  ...
  it('hasPassword', function() {
    ...
    hasPassword(req, res, next).should.equal(
      false,
      'Passed hasPassword while req.body.password is not a string'
    );
    // Invalid request (missing req.body.password)
    delete req.body.password;
    hasPassword(req, res, next).should.equal(
      false,
      'Passed hasPassword while missing req.body.password'
    );
  });
  ...
  it('hasTargetId', function() {
    ...
    req.body.targetId = '1337';
    hasTargetId(req, res, next).should.equal(
      false,
      'Passed hasTargetId with req.body.targetId not a number'
    );
    // Invalid request (missing req.body.targetId)
    delete req.body.targetId;
    hasTargetId(req, res, next).should.equal(
      false,
      'Passed hasTargetId while missing req.body.targetId'
    );
  });
  ...
  it('hasTargetType', function() {
    ...
    hasTargetType(req, res, next).should.equal(
      false,
      'Passed hasTargetType while req.body.targetType is not a string'
    );
    // Invalid request (missing req.body.targetType);
    delete req.body.targetType;
    hasTargetType(req, res, next).should.equal(
      false,
      'Passed hasTargetType while missing req.body.targetType'
    );
  });
  ...
  it('isLoggedIn', function() {
    ...
    req.session.usr = '4';
    isLoggedIn(req, res, next).should.equal(
      false,
      "Passed isLoggedIn with typeof(session.usr) = 'string' "
    );
    // Invalid request (missing usr)
    delete req.session.usr;
    isLoggedIn(req, res, next).should.equal(
      false,
      'Passed isLoggedIn while req.session has no .usr property'
    );
    ...
    isLoggedIn(req, res, next).should.equal(
      false,
      'Passed isLoggedIn while req.session has no .loggedIn property'
    );
    // Invalid request (loggedIn = false)
    req.session.loggedIn = false;
    isLoggedIn(req, res, next).should.equal(
      false,
      'Passed isLoggedIn while req.session.loggedIn = false'
    );
  });
  ...
  it('isInGame', function() {
    ...
    isInGame(req, res, next).should.equal(
      false,
      'Passed isInGame while missing session.game property'
    );
